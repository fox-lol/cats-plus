...
abstract class CatEntityMixin extends TameableEntity implements AnimatableContainer<ReplacedCatEntity>, EntityAccess, HoldableTickable {
    ...
    public void catsPlus$heldTick(PlayerEntity holder) {
        ...
        if (holderAccess.catsPlus$isInteractingWithHeldEntity()) {
            if (heldSoundCooldown <= 0) {
                holder.getWorld().playSoundFromEntity(null, holder, SoundEvents.ENTITY_CAT_PURR, getSoundCategory(), 0.6F + 0.4F * (random.nextFloat() - random.nextFloat()), 1);
                heldSoundCooldown = 5;
            }
        }
    }
